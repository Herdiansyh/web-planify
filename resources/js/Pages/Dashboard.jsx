...
export default function Dashboard({ tasks, count, page_settings, productivity_chart }) {
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 right-0 hidden w-96 overflow-y-auto border-l border-gray-200 px-4 py-6 dark:bg-gray-800 lg:px-8 xl:block">
        {/* card */}
        <Card className="mb-4">
          <CardContent className="mt-4">
            <div className="flex flex-col items-center">
              ...
              <span className="-mt1 text-sm font-light text-muted-foreground">{auth.email}</span>
            </div>
          </CardContent>
        </Card>
        <Card>
              ...
              <Link key={index} href={task.memberable.detail} className="hover:text-red-500">
                <div className="mb-4 flex items-center gap-x-2">
                  ...
                  <div className="flex flex-col items-start">
                    <span className="line-clamp-1 text-base font-medium leading-relaxed">{task.memberable.title}</span>
                    <span className="text-xs font-light text-muted-foreground">{task.memberable.created_at}</span>
                  </div>
                </div>
              </Link>
            ))}
